...
def _interpolate(low: Entry, high: Entry, intensity: float, bits: int) -> int:
    """
...
def intensity_to_value(channel: int, intensity: float, bits: int = BITS_PER_RGB) -> int:
    """
...
def _sine(x: float) -> float:
    ...
    y = x / _FOURTH_PI
    square = y * y
    return y * (_polynomial(_SIN_TOP, square) / _polynomial(_SIN_BOTTOM, square))
